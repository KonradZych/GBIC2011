...
toGenotypes <- function(ril, use=c("real","simulated","map"), treshold=0.01, overlapInd = 0, proportion = c(50,50), margin = 15, minChrLength = 0, verbose=FALSE, debugMode=0,...){
	#*******CHECKS*******
	require(qtl)
	if(any(proportion < 1) || any(proportion > 99)) stop("Proportion is a percentage (1,99)")
	#if(overlapInd < 0 || overlapInd > ncol(expressionMatrix)) stop("overlapInd is a number (0,lenght of the row).")
	if(margin < 0 || any(margin > proportion)) stop("Margin is a percentage (0,proportion)")
...
convertToGenotypes.internal <- function(ril,treshold,verbose=FALSE,debugMode=0){
	...
	for(x in rownames(upRils)){
		m <- rbind(m,splitRow.internal(x,upRils,upParental,ril$parental$groups,genotypes))
	}
	c<-0
	for(x in rownames(downRils)){
		m <- rbind(m,splitRow.internal(x,downRils,downParental,ril$parental$groups,genotypes))
	}
