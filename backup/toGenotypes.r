...
toGenotypes <- function(expressionMatrix, parentalExpression,  splitFUN = zero, overlapInd = 0, proportion = 50, margin = 5, genotypes = c(0,1), verbose=FALSE, debugMode=0){
	s <- proc.time()
...
childrenSplit <- function(expressionChildren,expressionParental){
	genoUp <- toGenotypes(expressionChildren[[2]],expressionParental[[2]],splitFUN=mean)
	genoDown <- toGenotypes(expressionChildren[[3]],expressionParental[[3]],splitFUN=mean)
...
parentalMean <- function(x){
	correctRows <- c(expressionParental[[2]],expressionParental[[3]])
	parentalMatrix <- expressionParental[[1]][correctRows,]
	result <- apply(parentalMatrix,1,mean)
	invisible(result)
}
...
checkExpression <- function(x, parentalExpressions, splitFUN, overlapInd, proportion, margin){
	r <- FALSE
